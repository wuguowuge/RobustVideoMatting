...
class Exporter:
    ...
    def export(self):
            # rec1 = torch.zeros([1,16,135,240]).to(self.args.device, self.precision)
            # rec2 = torch.zeros([1,20,68,120]).to(self.args.device, self.precision)
            # rec3 = torch.zeros([1,40,34,60]).to(self.args.device, self.precision)
            # rec4 = torch.zeros([1,64,17,30]).to(self.args.device, self.precision)
            # src = torch.randn([1,3,1080,1920]).to(self.args.device, self.precision)

            rec1 = torch.zeros([1,16,144,256]).to(self.args.device, self.precision)
            rec2 = torch.zeros([1,20,72,128]).to(self.args.device, self.precision)
            rec3 = torch.zeros([1,40,36,64]).to(self.args.device, self.precision)
            rec4 = torch.zeros([1,64,18,32]).to(self.args.device, self.precision)
            src = torch.randn([1,3,288,512]).to(self.args.device, self.precision)
            
            torch.onnx.export(
                self.model,
                (src, rec1, rec2, rec3, rec4),
                self.args.output,
                export_params=True,
                opset_version=self.args.opset,
                do_constant_folding=True,
